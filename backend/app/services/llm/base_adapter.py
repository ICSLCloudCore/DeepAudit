...
class BaseLLMAdapter(ABC):
    ...
    async def validate_config(self) -> bool:
        ...
        if not self.config.api_key:
            raise LLMError("API Key未配置", self.config.provider)
        return True
    ...
    async def with_timeout(self, coro, timeout_seconds: Optional[int] = None) -> Any:
        ...
        try:
            ...
            raise LLMError(f"请求超时 ({timeout}s)", self.config.provider)
    ...
    def handle_error(self, error: Any, context: str = "", api_response: str = None) -> None:
        ...
        # 针对不同错误类型提供更详细的信息
        if (
            "ConnectError" in str(type(error).__name__)
            or "连接" in message
            ...
            message = (
                f"无法连接到API服务器。建议：\n"
                f"1. 检查网络连接是否正常\n"
                f"2. 如果在中国大陆，可能需要配置代理访问海外API\n"
                f"3. 尝试切换到其他LLM提供商（如通义千问、DeepSeek等国内服务商）\n"
                f"4. 验证API端点地址是否正确"
            )
            status_code = status_code or 503
        elif "超时" in message or "timeout" in message.lower():
            message = (
                f"请求超时 ({self.config.timeout}s)。建议：\n"
                f"1. 检查网络连接是否正常\n"
                f"2. 尝试增加超时时间\n"
                f"3. 验证API端点是否正确"
            )
            ...
            keyword in message
            for keyword in ["余额不足", "资源包", "充值", "quota", "insufficient", "balance"]
        ):
        ...
        raise LLMError(
            full_message, self.config.provider, status_code, error, api_response=api_response
        )

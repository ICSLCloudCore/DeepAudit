...
async def export_database(
    ...
    try:
            ...
            "projects": [
                {
                    ...
                    "programming_languages": json.loads(p.programming_languages)
            ...
            "tasks": [
                {
                    ...
                    "exclude_patterns": json.loads(t.exclude_patterns)
                    if t.exclude_patterns
                    else [],
        ...
        return DatabaseExportResponse(
            export_date=export_data["export_date"], user_id=current_user.id, data=export_data
        )
...
async def import_database(
    ...
    try:
            ...
            for p_data in data["projects"]:
                ...
                if not existing:
                    project = Project(
                        ...
                        else (p_data.get("status") != "inactive" if "status" in p_data else True),
                    )
                    db.add(project)
        ...
        return {"message": "数据导入成功", "imported": imported_count}
...
async def clear_database(
    ...
    try:
        ...
        return {"message": "数据已清空", "deleted": deleted_count}
...
async def check_database_health(
    ...
    try:
        ...
        if database_connected:
            ...
            try:
                projects_count = len(
                    (await db.execute(select(Project).where(Project.owner_id == current_user.id)))
                    .scalars()
                    .all()
                )
                ...
                analyses_count = len(
                    (
                        await db.execute(
                            select(InstantAnalysis).where(
                                InstantAnalysis.user_id == current_user.id
                            )
                        )
                    )
                    .scalars()

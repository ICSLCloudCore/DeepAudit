    ...
    async with db_session_factory() as db:
        ...
        try:
            ...
            if project_id:
                ...
                zip_password = await get_project_zip_password(project_id)
            ...
            for root, dirs, files in os.walk(extract_dir):
                ...
                for file in files:
                    ...
                    if is_text_file(rel_path) and not should_exclude(
                        rel_path, custom_exclude_patterns
                    ):
                        try:
...
async def scan_zip(
    ...
    # 保存ZIP文件到持久化存储
    await save_project_zip(project_id, file_path, file.filename, password)
...
async def scan_stored_zip(
    ...
    background_tasks.add_task(
        process_zip_task, task.id, stored_zip_path, AsyncSessionLocal, user_config, project_id
    )
    ...
    try:
        if req.custom_prompt and req.custom_prompt.strip():
            ...
            result = await llm_service.analyze_code_with_custom_prompt(
                req.code, req.language, custom_prompt=req.custom_prompt.strip()
            )
        else:

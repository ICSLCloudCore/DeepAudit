...
def upgrade() -> None:
    ...
    if "files_with_findings" not in columns:
        op.add_column(
            "agent_tasks", sa.Column("files_with_findings", sa.Integer(), nullable=True, default=0)
        )
        # Set default value for existing rows
        op.execute(
            "UPDATE agent_tasks SET files_with_findings = 0 WHERE files_with_findings IS NULL"
        )
